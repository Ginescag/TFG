...
El desarrollo de este Trabajo ha permitido construir un sistema robótico de
patrulla autónoma funcional. Retomando los objetivos específicos formulados en
...
desplegable mediante contenedores se ha completado y validado de extremo a extremo en
simulación (OE9).

Conviene diferenciar el nivel de evidencia que respalda cada afirmación. Todo el ciclo se ha
validado de extremo a extremo en simulación, sobre la misma pila de software que ejecutaría el
robot. Además, una parte del sistema se ejercitó sobre el \mbox{TurtleBot~4} físico, donde la
autenticación, la telemetría, la cadena de mando del orquestador y la activación de la navegación
funcionaron sin modificar el software (Sección~\ref{sec:pruebas-real}). Lo que no llegó a
completarse fue una evaluación cuantitativa en condiciones reales, limitada por factores externos
al proyecto y planteada como trabajo futuro.
%% =============================================================================
...
El trabajo presenta varias limitaciones que conviene reconocer. La estrategia de
planificación de la ruta de cobertura es deliberadamente sencilla (un barrido en zig-zag) y
puede resultar subóptima en estancias de geometría compleja. La detección se limita
actualmente a la presencia de personas, sin seguimiento de objetos ni distinción entre
individuos, lo que puede generar alertas redundantes ante una misma persona. Además, la
percepción depende de una única cámara frontal, de modo que el robot conserva ángulos ciegos
a los lados y en la parte trasera durante la patrulla.

En el tratamiento de las incidencias subsiste otra limitación: el sistema se queda en
detectar la presencia de una persona, sin clasificar ni describir el suceso. Para
enriquecer ese tratamiento se ingenió e intentó integrar un \gls{vlm} ligero,
Qwen2.5-VL-3B-Instruct cuantizado a 4 bits, servido con vLLM a través de una API
compatible con OpenAI, que clasificara y describiera en lenguaje
natural cada incidencia a partir de su imagen. Sin embargo, no fue posible llevarlo a
producción: el servicio de inferencia exige una \acrshort{gpu} con memoria suficiente, y el
hardware del servidor no podía alojarlo a la vez que el resto del sistema (backend, bases de
datos, mensajería y visualización), mientras que ejecutarlo en el propio robot tampoco era
viable por sus recursos limitados. La integración se retoma como línea de trabajo futuro.
...
\begin{itemize}
  ...
  \item \textbf{Notificaciones en tiempo real.} Avisar al usuario de forma inmediata ante
    ...
    real.
  \item \textbf{Cobertura visual con cámara 360\textdegree.} Sustituir o complementar la
    cámara frontal por una cámara de 360\textdegree{} que elimine los ángulos ciegos del
    robot durante la patrulla.
  \item \textbf{Clasificación y descripción de incidencias con un \gls{vlm}.} Retomar la
    integración del modelo de lenguaje y visión ligero (Qwen2.5-VL servido con vLLM) sobre
    hardware de inferencia dedicado o un servicio externo, para enriquecer cada incidencia
    con una etiqueta y una descripción en lenguaje natural, a partir del intento descrito en
    la Sección~\ref{sec:limitaciones}.

    En relación con esta última línea, el extracto~\ref{lst:vllm} recoge la configuración del
servicio de inferencia con vLLM (\texttt{vllm/docker-compose.yml}) que se preparó durante el
intento descrito en la Sección~\ref{sec:limitaciones} y que constituye el punto de partida
para retomarla.

\SuspendTagging{code}%
\begin{yamlcode}[title={Servicio de inferencia con vLLM (vllm/docker-compose.yml)}, label={lst:vllm}]
services:
  vllm:
    image: vllm/vllm-openai:latest
    container_name: qwen25-vl-3b
    runtime: nvidia
    deploy:
      resources:
        reservations:
...
y facilitaron cada incorporación sucesiva. Llevar el sistema hasta completar su validación
sobre hardware real y su evaluación cuantitativa es el paso natural con el que me
